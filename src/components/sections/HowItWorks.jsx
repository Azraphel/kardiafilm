...
const steps = [
  {
    num: '01', tagColor: '#a855f7', tag: 'Réponse immédiate sur tous les canaux',
    title: 'Un client vous contacte — peu importe où',
    desc: "Appel, SMS, Facebook, Instagram, Google ou email. L'IA répond en quelques secondes, avec votre ton et votre style. Le client se sent pris en charge. Immédiatement.",
  },
  {
    num: '02', tagColor: '#f59e0b', tag: 'Filtrage automatique',
    title: "L'IA qualifie le lead",
    desc: "Elle pose les bonnes questions : type de service, superficie, localisation, budget, urgence. Les curieux sont filtrés. Vous ne parlez qu'aux clients sérieux.",
  },
  {
    num: '03', tagColor: '#a855f7', tag: 'Booking sans friction',
    title: 'Elle réserve dans votre calendrier',
    desc: "L'IA voit vos disponibilités en temps réel et confirme le rendez-vous directement. Le client reçoit une confirmation. Zéro intervention de votre part.",
  },
  {
    num: '04', tagColor: '#f59e0b', tag: 'No-shows éliminés',
    title: 'Elle collecte un dépôt ou une carte',
    desc: "Avant de terminer la conversation, l'IA sécurise un engagement financier. Un client qui a mis sa carte ne fait pas faux bond. Votre taux de no-show chute sous 5%.",
  },
  {
    num: '05', tagColor: '#a855f7', tag: 'Vous restez en contrôle',
    title: 'Vous recevez un résumé complet',
    desc: "Après chaque interaction : nom, canal utilisé, service demandé, date du RDV, infos collectées. Tout ça pendant que vous faisiez autre chose. Vous gérez votre compagnie, pas vos messages.",
  },
];
  ...
  <div style={{ background: 'linear-gradient(to bottom, #000, rgba(15,0,35,0.8), #000)' }}>
    ...
    <div style={{ textAlign: 'center', padding: 'clamp(3.5rem,7vw,6rem) clamp(1rem,4vw,1.5rem) clamp(2rem,4vw,3rem)' }}>
      <p style={{ fontFamily: 'monospace', fontSize: 'clamp(0.6rem,1.3vw,0.65rem)', color: 'rgba(168,85,247,0.6)', textTransform: 'uppercase', letterSpacing: '0.2em', marginBottom: '0.875rem' }}>
        ...
        De la prise de contact au RDV confirmé.{' '}
    ...
    {/* Stack */}
    <div style={{ paddingBottom: 'clamp(3rem,6vw,6rem)' }}>
      {steps.map((step, i) => (
        <div key={i} style={{
          position: 'sticky',
          ...
          <div style={{
            ...
            borderRadius: 'clamp(16px,2.5vw,24px)', overflow: 'hidden',
            position: 'relative',
            background: i % 2 === 0
            ...
            <div style={{ position: 'absolute', right: 'clamp(1rem,4vw,2.5rem)', bottom: '-0.5rem', fontFamily: 'monospace', fontSize: 'clamp(5rem,18vw,14rem)', fontWeight: 900, lineHeight: 1, color: `${step.tagColor}07`, userSelect: 'none', pointerEvents: 'none', letterSpacing: '-0.05em' }}>
              {step.num}
            </div>
            ...
            <div style={{ position: 'relative', zIndex: 1, padding: 'clamp(1.75rem,4vw,3.5rem)' }}>
              ...
              <h3 style={{ fontSize: 'clamp(1.35rem,4vw,2.75rem)', fontWeight: 800, color: 'white', letterSpacing: '-0.025em', lineHeight: 1.1, marginBottom: 'clamp(0.75rem,2vw,1.25rem)' }}>
                {step.title}
              </h3>
              <p style={{ fontSize: 'clamp(0.9rem,1.8vw,1.1rem)', color: 'rgba(255,255,255,0.42)', lineHeight: 1.8, maxWidth: '540px', marginBottom: 'clamp(1rem,2.5vw,1.75rem)' }}>
                {step.desc}
              </p>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.45rem', padding: 'clamp(0.275rem,0.7vw,0.4rem) clamp(0.75rem,1.8vw,1.1rem)', background: `${step.tagColor}0e`, border: `1px solid ${step.tagColor}28`, borderRadius: '999px' }}>
    ...
    <div style={{ textAlign: 'center', padding: 'clamp(3rem,6vw,6rem) clamp(1rem,4vw,1.5rem)', position: 'relative', overflow: 'hidden' }}>
      ...
      <a href="#contact" style={{
        display: 'inline-block', padding: 'clamp(0.875rem,2vw,1.125rem) clamp(1.75rem,4vw,2.5rem)',
        background: 'linear-gradient(135deg, #7c3aed, #6d28d9)', borderRadius: '999px',
        color: 'white', fontWeight: 700, fontSize: 'clamp(0.9rem,2vw,1.05rem)',
        textDecoration: 'none', boxShadow: '0 0 50px -8px rgba(124,58,237,0.6)',
        border: '1px solid rgba(168,85,247,0.3)', transition: 'transform 0.2s, box-shadow 0.2s',
      }}
